...
// Run: node tools/make-artifact.mjs [--reuse=true] [--demo] [--out=name.html]
//
// --demo bakes in the auto-running verification suite (for the Pages demo);
// --out overrides the output filename inside dist/ (default z3-artifact.html).
//
...
const outName = (process.argv.find((a) => a.startsWith("--out=")) || "--out=z3-artifact.html").slice(6);
...
const html = readFileSync(join(root, "tools", "artifact-template.html"), "utf8")
  ...
  .replaceAll("__DEMO__", String(demo))
  .replace("__WASM_B64__", () => b64)     // function form: payload may contain "$&" etc.
  .replace("__GLUE_JS__", () => glue);

for (const marker of ["__WASM_B64__", "__GLUE_JS__", "__Z3_VERSION__", "__DEMO__", "__REUSE_INSTANCE__"]) {
  if (html.includes(marker)) { console.error(`FAIL: unreplaced marker ${marker}`); process.exit(1); }
}

const outPath = join(dist, outName);
writeFileSync(outPath, html);
console.log(`${outName}: ${mb(html.length)} (wasm ${mb(wasm.length)} -> gz ${mb(gz.length)} -> b64 ${mb(b64.length)}, glue ${mb(glue.length)}) reuse=${reuse} demo=${demo}`);
