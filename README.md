...
- `test/perf-check.mjs` + `test/perf-suite.smt2` + `test/perf-baseline.json` — solver-level performance gate: a fixed 600-block SMT-LIB workload (LIA, bitvectors, arrays, UF, pigeonhole) run through both the CLI and the C API, min-of-N, failing on a >15% regression against the committed per-environment baseline. It also checks the verdict mix (200 sat / 400 unsat), so a solver erroring out early can't masquerade as a speedup, and refuses to compare across a changed workload (the baseline pins the suite's SHA-256). Each CI run publishes its measurement to `https://rupnikj.github.io/z3-inline/perf-result.json`.
...
Why 4.16.0 rather than the newer 5.0.0: Dafny bundles and defaults to Z3 4.16.0 on master/nightly (dafny-lang/dafny#6477), so this pin matches the solver the consuming Dafny build is tested against — and Dafny verification outcomes are famously Z3-version-sensitive (dafny-lang/dafny#6481). Z3 5.0.0 has no presence in the Dafny ecosystem (no solver-builds artifact, no upstream validation). The build always uses **native wasm exception handling**, patching out the legacy JS-based EH that Z3 4.x hardcodes into libz3's link flags (5.x already moved to wasm EH). This is worth the patch: measured on `test/perf-suite.smt2`, legacy JS EH costs **2.04× solve time (1823 ms vs 895 ms) and +3.9 MB of wasm** for byte-identical verdicts, because every call that can throw round-trips wasm→JS→wasm through one of 72 `invoke_*` trampolines. With wasm EH, Z3 4.16.0 is both smaller and marginally faster than the 5.0.0 build it replaced (10.39 MB / 895 ms vs 10.74 MB / 911 ms) — so the version pin costs nothing. The ABI is recorded as `exceptionAbi` in `build-info.json`, and the build fails rather than silently producing the slow variant if it cannot remove the flag.
